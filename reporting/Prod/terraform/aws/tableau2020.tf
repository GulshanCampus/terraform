...
/*resource "aws_instance" "tab2020v4p" {
...
data "template_file" "tab2020v4w1" {
  template = file("userdata/tableau.txt")

  vars = {
    hostname = "tab2020v4w1"
    worker   = "worker1"
  }
}

/*resource "aws_instance" "tab2020v4w1" {
  ami                         = data.aws_ami.Windows-2019.image_id
  instance_type               = "c5.4xlarge"
  ebs_optimized               = true
...
data "template_file" "tab2020v4w2" {
  template = file("userdata/tableau.txt")

  vars = {
    hostname = "tab2020v4w2"
    worker   = "worker2"
  }
}

/*resource "aws_instance" "tab2020v4w2" {
  ami                         = data.aws_ami.Windows-2019.image_id
  instance_type               = "c5.4xlarge"
  ebs_optimized               = true
